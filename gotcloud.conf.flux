...
[cleanUpBam]
CMD = $(CRAM_VIEW) ?(BAM) |  $(BAM_EXE) squeeze --in -.ubam --keepDups --rmTags AS:i,BD:Z,BI:Z,XS:i,MC:Z,MD:Z,NM:i,MQ:i --out - | $(SAMTOOLS_EXE) view -S -b -F 0x800 - | $(SAMTOOLS_SORT_EXE) sort -n -T $(DIR)/?(BAM).temp - | $(SAMTOOLS_EXE) fixmate - $(OUTPUT) 2> $(OUTPUT)2fastq.log
...
DIR = $(OUT_DIR)/fastqs
OUTPUT = $(DIR)/?(BAM)
CMD = $(CRAM_VIEW) ?(BAM) | $(BAM_EXE) bam2fastq --in -.ubam --outBase $(OUTPUT) --splitRG --readname --gzip 2> $(OUTPUT)2fastq.log
